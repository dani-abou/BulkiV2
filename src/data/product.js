export const PRODUCTS = {
  'Primabull Ancestral Ground Beef': {
    name: 'Primabull Ancestral Ground Beef',
    price: 10,
    unit: '1lb',
    image: '/products/ground.webp',
    description: [`For those who want to benefit of heart and liver, but don’t like the taste. Comes in 1lb package`,
      '',
      `
      - 80% Ground Beef`,
      `- 20% Ground beef heart and liver`,
      `Includes: + Daily serving of iron, riboflavin, vitamin b12, vitamin A, and copper`,
      ``,
      `107 + grams of protein / lb`
    ]
  },
  'Pork Kielbasa': {
    name: 'Pork Kielbasa',
    price: 12,
    unit: '4 links',
    image: '/products/kielbasa.webp',
    description: [`Grilled, roasted or sautéed, these well seasoned links have an addition of garlic and marjoram making them a full flavored treat.`,
      `4 links per package, 1.00 - 1.25 lbs`
    ]
  },
  'Beef Liver': {
    name: 'Beef Liver',
    price: 12,
    unit: '4 links',
    image: '/products/liver.webp',
    description: [`Includes: + Daily serving of iron, riboflavin, vitamin b12, vitamin A, and copper`, `
    1 lb package of grass fed, grass finished beef liver`
    ]
  },

  'Ancestral Beef Bundle': {
    name: 'Ancestral Beef Bundle',
    price: 200,
    unit: '4 links',
    image: '/products/ground.webp',
    description: [
      `25 lbs of our 80% Beef/ 20% heart and liver blend. Comes in 1lb packages.`,
      `Ships for free!`,
      `Includes: Iron, riboflavin, vitamin b12, vitamin A, and copper`,
      `2,700 grams protein +`
    ]
  },
  '1/8 Cow Share': {
    name: '1/8 Cow Share',
    price: 550,
    unit: '4 links',
    image: '/products/eighth.jpg',
    description: [`Includes: Prcoessing fees and free shipping, 40lbs of beef, $5.50/ meal, $13.75/lbs`,
      `24 lbs of individually packaged ground beef`,
      `1 Roast(Eye, Chuck or Top Round)`,
      `2 Shank Packages(Soup Bones)`,
      `2 Stew Meat Packages`,
      `1 of the following: Brisket or Short Ribs`,
      `1 Sirloin Tip`,
      `1 Sirloin`,
      `2 - 3 Ribeye`,
      `2 Filet Mignon`,
      `3 New York Strip`,
      `1 of the following: Flank, Skirt or Flap Steak`
    ]
  },
  'Taste of the farm': {
    name: 'Taste of the farm',
    price: 159,
    unit: '',
    image: '/products/tasteOfFarm.webp',
    description: [
      `1 Pastured Raised Bone-in Skin on Half Chicken`,
      `4 Pastured Raised Chicken Italian Sausage Links`,
      `1 Package Pastured Raised Chicken Tenders`,
      `2 Boneless Pork Chops`,
      `1lb Ground Pork`,
      `4 Pork sweet Italian sausage Links`,
      `1 Dry - Aged Beef Sirloin Steak`,
      `1 Dry - Aged Beef T-Bone Steak`,
      `1lb Dry - Aged Beef Stew`,
      `2lbs  Dry - Aged Ground Beef`,
      `2 Dry - Aged Beef Short rib Brisket Burgers`,
      `Bonus Free 1 Package Pastured raised chicken drumsticks`
    ]
  },

}